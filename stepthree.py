...
def find_motifs():
    ...
    for i in range(0, seq_size-ML + 1):
        for j in range(0, seq_size-ML + 1):
            for k in range(0, seq_size-ML + 1):
                test_motif_A = seq[1][i:i + ML] # the slice represents the size of the motif
                test_motif_B = seq[3][j:j + ML]
                test_motif_C = seq[5][k:k + ML]
                if score(test_motif_A, test_motif_B) > score(m[0], m[1]):
                    m[0] = test_motif_A
                    m[1] = test_motif_B
                    pred_sites[0] = i
                    pred_sites[1] = j
                if score(test_motif_A, test_motif_C) > score(m[0], m[2]):
                    m[0] = test_motif_A
                    m[2] = test_motif_C
                    pred_sites[0] = i
                    pred_sites[2] = k

    for i in range(7, len(seq), 2): # we iterate all sequences beyond the first two
        for j in range(0, seq_size-ML+1):
            tmot = seq[i][j:j + ML]
